  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto py-6 md:py-12 px-4 md:px-6 w-full space-y-8 md:space-y-12">
        <Link href="/pitches" className="flex items-center gap-2 text-muted-foreground hover:text-primary transition-all w-fit font-black text-[10px] md:text-xs uppercase tracking-widest group">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 items-start">
          <div className="lg:col-span-8 space-y-8 md:space-y-10">
            <Card className="border-none shadow-xl md:shadow-3xl overflow-hidden bg-white rounded-[2rem] md:rounded-[3rem] relative transition-all">
              <div className="relative aspect-[16/9] md:aspect-[21/10] w-full overflow-hidden bg-muted/30">
                ...
                <div className="absolute bottom-6 left-6 right-6 md:bottom-10 md:left-10 md:right-10 flex items-end justify-between">
                   <div className="space-y-3 md:space-y-5">
                    <Badge variant="outline" className="bg-black/40 backdrop-blur-md text-white border-white/20 px-4 md:px-6 py-1.5 md:py-2 text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] rounded-lg md:rounded-xl shadow-2xl">
                      {pitch.category || pitch.industry || 'Other'}
                    </Badge>
                    <h1 className="text-3xl md:text-7xl font-black tracking-tighter leading-none text-white drop-shadow-2xl">{pitch.startupName}</h1>
                   </div>
                </div>
              </div>
              ...
              <CardHeader className="p-6 md:p-12 pb-6 md:pb-8 space-y-6 md:space-y-10">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex flex-wrap items-center gap-4 md:gap-8 text-[8px] md:text-[10px] text-muted-foreground font-black uppercase tracking-[0.2em] md:tracking-[0.3em]">
                    <span className="flex items-center gap-2 px-3 md:px-4 py-1.5 md:py-2 bg-muted/40 rounded-full"><Clock className="w-3 h-3 md:w-4 md:h-4 text-primary" /> Active</span>
                    <span className="flex items-center gap-2 px-3 md:px-4 py-1.5 md:py-2 bg-emerald-50 text-emerald-700 rounded-full"><ShieldCheck className="w-3 h-3 md:w-4 md:h-4 text-emerald-600" /> Verified</span>
                  </div>
                  <div className="flex items-center gap-2 md:gap-4">
                    {isOwner && profile?.role === 'startup' && (
                      <Button 
                        variant="ghost" 
                        size="sm" 
                        className="text-destructive hover:bg-destructive/10 h-10 md:h-12 px-4 md:px-6 rounded-xl md:rounded-2xl font-black uppercase text-[8px] md:text-[10px] tracking-widest"
                        onClick={handleDeletePitch}
                        disabled={checking}
                      >
                        {checking ? <Loader2 className="animate-spin w-3 h-3 md:w-4 md:h-4 md:mr-2" /> : <Trash2 className="w-3 h-3 md:w-4 md:h-4 md:mr-2" />}
                      ...
                      <div className="flex items-center gap-2 md:gap-4 w-full sm:w-auto">
                         {(isInterested || (contactRequest && contactRequest.status === 'accepted')) && (
                           <Button 
                             variant="outline" 
                             size="sm" 
                             className="flex-1 sm:flex-none text-amber-600 border-amber-200 hover:bg-amber-50 h-10 md:h-12 px-4 md:px-6 rounded-xl md:rounded-2xl font-black uppercase text-[8px] md:text-[10px] tracking-widest border-2 shadow-sm"
                             onClick={(e) => handleResolveConnection(e)}
                             disabled={resolving}
                           >
                             {resolving ? <Loader2 className="animate-spin w-3 h-3 md:w-4 md:h-4 md:mr-2" /> : <Zap className="w-3 h-3 md:w-4 md:h-4 md:mr-2" />}
                         ...
                         <Button 
                           ...
                           className={cn(
                             "flex-1 sm:flex-none h-10 md:h-12 px-4 md:px-8 rounded-xl md:rounded-2xl border-2 font-black uppercase text-[8px] md:text-[10px] tracking-widest transition-all flex gap-2 md:gap-3 items-center shadow-sm",
              ...
              <CardContent className="p-6 md:p-12 pt-0 space-y-8 md:space-y-12">
                <div className="p-6 md:p-10 bg-muted/20 rounded-[2rem] md:rounded-[3rem] border-l-4 md:border-l-8 border-primary relative overflow-hidden shadow-inner">
                  <Sparkles className="absolute -right-4 -top-4 md:-right-6 md:-top-6 w-20 h-20 md:w-32 md:h-32 text-primary/5 -rotate-12" />
                  <p className="text-xl md:text-4xl leading-relaxed text-foreground/90 font-medium italic relative z-10 tracking-tight">
                    &quot;{pitch.description}&quot;
                  </p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-8">
                  <div className="p-8 md:p-10 bg-primary shadow-xl md:shadow-2xl shadow-primary/20 rounded-[2rem] md:rounded-[2.5rem] space-y-3 md:space-y-4 flex flex-col items-center text-center text-white relative overflow-hidden group hover:scale-[1.02] transition-transform">
                    ...
                    <p className="text-[8px] md:text-[10px] uppercase tracking-[0.2em] md:tracking-[0.3em] font-black opacity-70">Target Capital</p>
                    <p className="text-3xl md:text-5xl font-black tracking-tighter">${pitch.fundingNeeded?.toLocaleString()}</p>
                  </div>
                  <div className="p-8 md:p-10 bg-white border-2 border-muted shadow-sm rounded-[2rem] md:rounded-[2.5rem] space-y-3 md:space-y-4 flex flex-col items-center text-center group hover:border-accent/20 transition-all hover:shadow-xl">
                    ...
                    <p className="text-[8px] md:text-[10px] uppercase tracking-[0.2em] md:tracking-[0.3em] text-muted-foreground font-black">Strategic Sector</p>
              ...
              {isInvestor && (
                <CardFooter className="p-6 md:p-12 border-t bg-muted/5 flex flex-col sm:flex-row gap-4 md:gap-8">
                   <div className="flex-[1.5] flex flex-col gap-3 md:gap-4 w-full">
                     {!contactRequest ? (
                       <Button className="w-full h-16 md:h-20 text-lg md:text-xl font-black shadow-xl md:shadow-2xl shadow-primary/20 bg-primary hover:bg-primary/90 rounded-xl md:rounded-[1.5rem] transition-all active:scale-95" onClick={handleRequestContact}>
                         <Mail className="mr-3 md:mr-4 w-6 h-6 md:w-7 md:h-7" /> Request Inquiry
                       </Button>
                     ) : contactRequest.status === 'pending' ? (
                       <Button className="w-full h-16 md:h-20 text-lg md:text-xl font-black rounded-xl md:rounded-[1.5rem] bg-muted/50 border-2 border-dashed text-muted-foreground" variant="secondary" disabled>
                         <Clock className="mr-3 md:mr-4 w-6 h-6 md:w-7 md:h-7 animate-pulse" /> Authentication
                       </Button>
                     ) : contactRequest.status === 'accepted' ? (
                       <div className="flex flex-col sm:flex-row gap-3 md:gap-4 w-full">
                          <Button className="flex-1 h-16 md:h-20 text-md md:text-lg font-black bg-emerald-600 hover:bg-emerald-700 shadow-xl md:shadow-2xl shadow-emerald-500/20 rounded-xl md:rounded-[1.5rem] transition-all active:scale-95" asChild>
                            <a href={`mailto:${pitch.contactEmail}`}>
                              <Mail className="mr-2 md:mr-3 w-5 h-5 md:w-6 md:h-6" /> Email Founder
                            </a>
                          </Button>
                          <Button className="flex-1 h-16 md:h-20 text-md md:text-lg font-black bg-accent hover:bg-accent/90 shadow-xl md:shadow-2xl shadow-accent/20 rounded-xl md:rounded-[1.5rem] transition-all active:scale-95" asChild>
                            <Link href="/messages">
                       ...
                       <Button className="w-full h-16 md:h-20 text-lg md:text-xl font-black rounded-xl md:rounded-[1.5rem] opacity-50" variant="outline" disabled>
                   ...
                   <Button 
                    className={cn(
                      "flex-1 h-16 md:h-20 text-lg md:text-xl font-black transition-all rounded-xl md:rounded-[1.5rem] shadow-xl md:shadow-2xl active:scale-95 w-full",
          ...
          <div className="lg:col-span-4 space-y-8 md:space-y-10">
            <Card className="border-none shadow-xl md:shadow-2xl bg-white overflow-hidden rounded-[2rem] md:rounded-[2.5rem] hover:shadow-2xl md:hover:shadow-3xl transition-all duration-500">
              <CardHeader className="bg-primary/5 border-b py-8 md:py-10 px-8 md:px-10">
                <CardTitle className="text-[10px] md:text-xs font-black flex items-center gap-3 text-primary uppercase tracking-[0.2em] md:tracking-[0.3em]">
              ...
              <CardContent className="p-8 md:p-10 space-y-8 md:space-y-10">
                <div className="flex items-center gap-4 md:gap-6 group">
                  <div className="w-16 h-16 md:w-24 md:h-24 bg-muted/30 rounded-2xl md:rounded-[2rem] flex items-center justify-center border-4 border-white shadow-lg md:shadow-xl relative overflow-hidden transition-transform duration-500 group-hover:scale-105">
                  ...
                  <div className="space-y-1 md:space-y-1.5 overflow-hidden">
                    ...
                    <p className="text-[8px] md:text-[9px] text-muted-foreground font-black uppercase tracking-widest">Verified Member</p>
                  </div>
                </div>

                <div className="space-y-4 md:space-y-5">
                  <div className="flex items-center gap-2 md:gap-3 text-[8px] md:text-[10px] font-black uppercase text-muted-foreground tracking-[0.2em]">
                  ...
                  <p className="text-xs md:text-sm text-muted-foreground leading-relaxed italic p-6 md:p-8 bg-muted/20 rounded-2xl md:rounded-[2rem] border-l-4 border-primary/20 shadow-inner">
                ...
                <Link href="/pitches" className="block group">
                  <Button variant="outline" className="w-full h-12 md:h-14 rounded-xl md:rounded-2xl border-2 font-black uppercase tracking-widest text-[8px] md:text-[10px] transition-all hover:bg-primary hover:text-white group-active:scale-95 shadow-sm">
                    Market Feed
                  </Button>
                </Link>
              </CardContent>
            ...
            <Card className="border-none shadow-xl md:shadow-2xl bg-accent text-white overflow-hidden rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-12 relative group">
              ...
              <div className="space-y-6 md:space-y-8 relative z-10">
                <Badge className="bg-white/20 text-white border-none font-black uppercase text-[8px] md:text-[9px] tracking-[0.2em] px-4 md:px-5 py-1.5 md:py-2 rounded-lg md:rounded-xl shadow-lg w-fit">Venture Protection</Badge>
