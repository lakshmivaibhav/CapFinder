          ...
          <div className="flex flex-col sm:flex-row gap-3 md:gap-4 w-full md:w-auto">
            {isStartup && (
              <Link href="/pitches/new" className="w-full sm:w-auto">
                <Button className="w-full gap-3 h-12 md:h-14 px-6 md:px-8 rounded-xl bg-primary shadow-xl shadow-primary/20 hover:shadow-primary/30 transition-all font-black uppercase tracking-widest text-[10px]">
            ...
            {(isInvestor || isAdmin) && (
              <Link href="/pitches" className="w-full sm:w-auto">
                <Button variant="outline" className="w-full gap-3 h-12 md:h-14 px-6 md:px-8 rounded-xl border-2 hover:bg-primary/5 transition-all font-black uppercase tracking-widest text-[10px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-8 mb-10 md:mb-16">
          <Card className="border-none shadow-xl bg-primary/5 rounded-[2rem] overflow-hidden relative group">
            ...
            <CardContent className="p-6 md:p-10 flex items-center gap-4 md:gap-8">
              <div className="w-12 h-12 md:w-16 md:h-16 bg-primary rounded-2xl flex items-center justify-center text-white shadow-2xl shadow-primary/30">
          ...
          <Card className="border-none shadow-xl bg-accent/5 rounded-[2rem] overflow-hidden relative group">
            ...
            <CardContent className="p-6 md:p-10 flex items-center gap-4 md:gap-8">
              <div className="w-12 h-12 md:w-16 md:h-16 bg-accent rounded-2xl flex items-center justify-center text-white shadow-2xl shadow-accent/30">
          ...
          <Card className="border-none shadow-xl bg-emerald-50 rounded-[2rem] overflow-hidden relative group sm:col-span-2 md:col-span-1">
            ...
            <CardContent className="p-6 md:p-10 flex items-center gap-4 md:gap-8">
              <div className="w-12 h-12 md:w-16 md:h-16 bg-emerald-500 rounded-2xl flex items-center justify-center text-white shadow-2xl shadow-emerald-500/30">
        ...
        {isInvestor && recommendedPitches.length > 0 && (
          <section className="mb-10 md:mb-16">
            <div className="flex items-center gap-3 md:gap-4 mb-6 md:mb-8">
              <div className="p-2 md:p-3 bg-accent/10 rounded-2xl">
                ...
                <Link key={pitch.id} href={`/startup/${pitch.id}`}>
                  <Card className="group hover:border-accent/30 border-2 border-transparent transition-all h-full shadow-xl hover:shadow-2xl rounded-[2rem] flex flex-col bg-white overflow-hidden">
                    <CardHeader className="p-6 md:p-8 pb-4">
                      <Badge className="w-fit bg-accent/10 text-accent border-none font-black text-[9px] uppercase tracking-[0.2em] mb-3 md:mb-4 px-3 md:px-4 py-1.5 rounded-lg">
                    ...
                    <CardFooter className="p-6 md:p-8 pt-0 flex justify-between items-center border-t border-muted/50 mt-4 bg-muted/5">
                       ...
                       <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-white shadow-md flex items-center justify-center group-hover:bg-accent group-hover:text-white transition-all duration-500 group-hover:translate-x-2">
        ...
        <Tabs defaultValue="primary" className="space-y-6 md:space-y-10">
          <div className="flex justify-between items-center border-b pb-4 md:pb-6">
            <TabsList className="bg-muted/50 p-1 md:p-1.5 rounded-xl md:rounded-2xl h-12 md:h-14 w-full sm:w-fit shadow-inner">
              <TabsTrigger value="primary" className="flex-1 sm:flex-none gap-2 md:gap-3 px-4 md:px-8 h-10 md:h-11 rounded-lg md:rounded-xl font-black uppercase text-[10px] tracking-widest data-[state=active]:bg-white data-[state=active]:shadow-lg transition-all">
                {isStartup ? <><Megaphone className="w-3 h-3 md:w-4 md:h-4" /> Active</> : <><LayoutGrid className="w-3 h-3 md:w-4 md:h-4" /> Feed</>}
              </TabsTrigger>
              <TabsTrigger value="secondary" className="flex-1 sm:flex-none gap-2 md:gap-3 px-4 md:px-8 h-10 md:h-11 rounded-lg md:rounded-xl font-black uppercase text-[10px] tracking-widest data-[state=active]:bg-white data-[state=active]:shadow-lg transition-all">
          ...
          <TabsContent value="primary" className="mt-0 outline-none">
            {(isStartup ? loadingStartupPitches : loadingAllPitches) ? (
                    ...
                    <Card key={pitch.id} className="relative group overflow-hidden border-none shadow-xl transition-all duration-500 rounded-[2rem] flex flex-col bg-white">
                      <CardHeader className="p-6 md:p-8 pb-4">
                        <div className="flex justify-between items-start mb-3 md:mb-4">
                          <Badge variant="outline" className="border-primary/20 text-primary font-black uppercase text-[9px] tracking-[0.2em] px-3 md:px-4 py-1.5 rounded-lg bg-primary/5">{pitch.category || pitch.industry || 'Other'}</Badge>
                          {hasActiveConnection && (
                            <Button 
                              variant="ghost" 
                              size="sm" 
                              className="h-8 md:h-10 px-3 md:px-4 text-[9px] font-black uppercase tracking-widest text-amber-600 hover:bg-amber-50 rounded-full z-10 border-2 border-amber-100 shadow-sm"
                              onClick={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
              ...
              <div className="text-center py-20 md:py-32 bg-muted/10 rounded-[2rem] border-2 md:border-4 border-dashed flex flex-col items-center p-6">
                ...
                {isStartup && (
                  <Link href="/pitches/new" className="mt-6 md:mt-8">
                    <Button variant="outline" className="rounded-xl px-8 md:px-10 h-12 md:h-14 border-2 font-black uppercase text-[10px] tracking-widest">Initialize Your First Pitch</Button>
                  </Link>
                )}
              </div>
            )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10">
                {(isStartup ? startupInterests : investorInterests)?.map((interest) => (
                  <Card key={interest.id} className="border-none shadow-xl transition-all duration-500 rounded-[2rem] bg-white group">
                    <CardHeader className="p-6 md:p-8 pb-4">
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground mb-2 md:mb-3">{isStartup ? 'Investor Identity' : 'Venture Opportunity'}</p>
                    ...
                    <CardContent className="p-6 md:p-8 pt-0">
                      <Badge variant="secondary" className="bg-primary/5 text-primary border-none text-[10px] font-black uppercase tracking-[0.2em] px-3 md:px-4 py-1.5 rounded-lg">{interest.industry}</Badge>
                    </CardContent>
                    <CardFooter className="p-6 md:p-8 pt-4 border-t border-muted/50 bg-muted/5">
                      <Link href={isStartup ? `/investor/${interest.investorId}` : `/startup/${interest.pitchId}`} className="w-full">
                        <Button variant="outline" className="w-full h-12 md:h-14 rounded-xl font-black uppercase text-[10px] tracking-widest border-2 hover:bg-primary/5 shadow-sm transition-all">Analyze Context</Button>
                      </Link>
                    </CardFooter>
                  </Card>
                ))}
