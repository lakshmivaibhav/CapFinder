      ...
      <main className="flex-1 p-4 md:p-10 max-w-7xl mx-auto w-full space-y-8 md:space-y-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b pb-8 md:pb-10 text-center md:text-left">
          <div className="space-y-2">
            <p className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-destructive">Restricted Access</p>
          ...
          <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200 h-10 px-4 rounded-xl font-bold uppercase tracking-widest text-[8px] md:text-[9px] w-fit mx-auto md:mx-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
          {stats.map((stat, i) => (
            <Card key={i} className="border-none shadow-md hover:shadow-lg transition-all relative overflow-hidden rounded-[1.5rem] md:rounded-[2rem]">
              <CardHeader className="pb-2">
                <CardDescription className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-muted-foreground mb-1">{stat.label}</CardDescription>
                <CardTitle className="text-3xl md:text-4xl font-black">{stat.value}</CardTitle>
              </CardHeader>
              <CardContent>
                <div className={`absolute right-4 bottom-4 md:right-6 md:bottom-6 p-3 md:p-4 rounded-xl md:rounded-2xl ${stat.bg} shadow-inner`}>
        ...
        <Tabs defaultValue="users" className="space-y-6 md:space-y-10">
          <ScrollArea className="w-full">
            <TabsList className="bg-muted/50 p-1 md:p-1.5 rounded-xl md:rounded-2xl h-12 md:h-14 w-fit min-w-full sm:min-w-0">
              <TabsTrigger value="users" className="gap-2 px-4 md:px-8 h-10 md:h-11 rounded-lg md:rounded-xl font-bold text-xs md:text-sm data-[state=active]:shadow-lg"><UserCog className="w-4 h-4" /> Identity</TabsTrigger>
              <TabsTrigger value="pitches" className="gap-2 px-4 md:px-8 h-10 md:h-11 rounded-lg md:rounded-xl font-bold text-xs md:text-sm data-[state=active]:shadow-lg"><Building className="w-4 h-4" /> Ventures</TabsTrigger>
              <TabsTrigger value="delete-requests" className="gap-2 px-4 md:px-8 h-10 md:h-11 rounded-lg md:rounded-xl font-bold text-xs md:text-sm data-[state=active]:shadow-lg relative">
          ...
          <TabsContent value="users">
            <Card className="border-none shadow-xl overflow-hidden rounded-[1.5rem] md:rounded-[2rem] bg-white">
              <ScrollArea className="w-full">
                <Table>
                  <TableHeader className="bg-muted/40">
                    <TableRow>
                      <TableHead className="px-6 md:px-8 font-black uppercase tracking-widest text-[8px] md:text-[10px] whitespace-nowrap">Professional Identity</TableHead>
                      <TableHead className="font-black uppercase tracking-widest text-[8px] md:text-[10px] whitespace-nowrap">Classification</TableHead>
                      <TableHead className="font-black uppercase tracking-widest text-[8px] md:text-[10px] whitespace-nowrap">Status</TableHead>
                      <TableHead className="font-black uppercase tracking-widest text-[8px] md:text-[10px] whitespace-nowrap">Verification</TableHead>
                      <TableHead className="text-right px-6 md:px-8 font-black uppercase tracking-widest text-[8px] md:text-[10px] whitespace-nowrap">Governance</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {loadingUsers ? (
                      ...
                      <TableRow key={u.id} className="hover:bg-muted/10 transition-colors">
                        <TableCell className="px-6 md:px-8">
                          <div className="flex flex-col gap-0.5">
                            ...
                            <span className="text-[8px] md:text-[10px] text-muted-foreground font-medium uppercase tracking-widest">{u.email}</span>
                          </div>
                        </TableCell>
                        <TableCell><Badge variant="outline" className="capitalize rounded-lg px-2 md:px-3 font-bold border-muted-foreground/20 text-[10px]">{(u.role as string)}</Badge></TableCell>
                        <TableCell>
                          {u.disabled ? <Badge variant="destructive" className="rounded-lg px-2 md:px-3 font-bold text-[10px]">Suspended</Badge> : <Badge variant="secondary" className="bg-emerald-50 text-emerald-700 rounded-lg px-2 md:px-3 font-bold text-[10px]">Compliant</Badge>}
                        </TableCell>
                        <TableCell>
                          {u.verified ? <Badge className="bg-emerald-500 text-white rounded-lg px-2 md:px-3 font-bold text-[10px]">Verified</Badge> : <Badge variant="outline" className="rounded-lg px-2 md:px-3 font-bold text-[10px]">Unverified</Badge>}
                        </TableCell>
                        <TableCell className="text-right px-6 md:px-8 space-x-1 whitespace-nowrap">
                          <Button variant="ghost" size="icon" title={u.verified ? "Unverify User" : "Verify User"} className="rounded-xl h-8 w-8" onClick={() => toggleVerificationStatus(u.id, u.email, !!u.verified)}>
                            <ShieldCheck className={cn("w-4 h-4 md:w-5 md:h-5", u.verified ? "text-emerald-600" : "text-muted-foreground")} />
                          </Button>
                          <Button variant="ghost" size="icon" className="rounded-xl h-8 w-8" onClick={() => toggleUserStatus(u.id, u.email, !!u.disabled)}>
                            {u.disabled ? <UserCheck className="w-4 h-4 md:w-5 md:h-5 text-emerald-600" /> : <UserX className="w-4 h-4 md:w-5 md:h-5 text-amber-600" />}
                          </Button>
                          <Button variant="ghost" size="icon" className="rounded-xl h-8 w-8 text-destructive hover:bg-destructive/10" onClick={() => handleDeleteUser(u.id, u.email)}>
            ...
            <Card className="border-none shadow-xl overflow-hidden rounded-[1.5rem] md:rounded-[2rem] bg-white">
              <ScrollArea className="w-full">
                <Table>
                  <TableHeader className="bg-muted/40">
                    <TableRow>
                      <TableHead className="px-6 md:px-8 font-black uppercase tracking-widest text-[8px] md:text-[10px] whitespace-nowrap">Venture Name</TableHead>
                      <TableHead className="font-black uppercase tracking-widest text-[8px] md:text-[10px] whitespace-nowrap">Capital Goal</TableHead>
                      <TableHead className="font-black uppercase tracking-widest text-[8px] md:text-[10px] whitespace-nowrap">Current Status</TableHead>
                      <TableHead className="text-right px-6 md:px-8 font-black uppercase tracking-widest text-[8px] md:text-[10px] whitespace-nowrap">Governance</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                      ...
                      <TableRow key={p.id} className="hover:bg-muted/10 transition-colors">
                        ...
                        <TableCell>
                          {(!p.status || p.status === 'approved') ? (
                            <Badge variant="secondary" className="bg-emerald-50 text-emerald-700 font-bold gap-1.5 rounded-lg px-2 md:px-3 text-[10px]">
                              <CheckCircle2 className="w-3 h-3" /> Approved
                            </Badge>
                          ) : p.status === 'pending' ? (
                            <Badge variant="outline" className="text-amber-600 border-amber-200 bg-amber-50 font-bold gap-1.5 rounded-lg px-2 md:px-3 text-[10px]">
                              <Clock className="w-3 h-3 animate-pulse" /> Pending
                            </Badge>
                          ) : (
                            <Badge variant="destructive" className="font-bold gap-1.5 rounded-lg px-2 md:px-3 text-[10px]">
                        ...
                        <TableCell className="text-right px-6 md:px-8 space-x-1 whitespace-nowrap">
                          <Button 
                            variant="ghost" 
                            size="icon" 
                            className="rounded-xl h-8 w-8 text-emerald-600 hover:bg-emerald-50" 
                          ...
                          <Button 
                            variant="ghost" 
                            size="icon" 
                            className="rounded-xl h-8 w-8 text-amber-600 hover:bg-amber-50" 
          ...
          <TabsContent value="delete-requests">
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                <h3 className="text-[8px] md:text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Pending Purge Operations</h3>
                {staleRequests.length > 0 && (
                  <Button variant="destructive" size="sm" className="w-full sm:w-auto rounded-xl font-bold h-10 px-6 shadow-lg shadow-destructive/20 text-xs" onClick={handleProcessStaleRequests} disabled={processingStale}>
              ...
              <Card className="border-none shadow-xl overflow-hidden rounded-[1.5rem] md:rounded-[2rem] bg-white">
                <ScrollArea className="w-full">
                  <Table>
                    <TableHeader className="bg-muted/40">
                      <TableRow>
                        <TableHead className="px-6 md:px-8 font-black uppercase tracking-widest text-[8px] md:text-[10px] whitespace-nowrap">Type</TableHead>
                        <TableHead className="font-black uppercase tracking-widest text-[8px] md:text-[10px] whitespace-nowrap">Entity ID</TableHead>
                        <TableHead className="font-black uppercase tracking-widest text-[8px] md:text-[10px] whitespace-nowrap">Initiated</TableHead>
                        <TableHead className="text-right px-6 md:px-8 font-black uppercase tracking-widest text-[8px] md:text-[10px] whitespace-nowrap">Decision</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {loadingDeleteRequests ? (
                        ...
                        <TableRow key={req.id} className="hover:bg-muted/10 transition-colors">
                          <TableCell className="px-6 md:px-8"><Badge variant="outline" className="capitalize font-black border-destructive/20 text-destructive bg-destructive/5 px-2 md:px-3 py-1 rounded-lg text-[10px]">{(req.targetType as string)}</Badge></TableCell>
                          <TableCell className="text-[8px] md:text-[10px] font-mono font-bold text-muted-foreground whitespace-nowrap">{(req.targetId as string)}</TableCell>
                          <TableCell className="text-[8px] md:text-[10px] font-black text-muted-foreground uppercase tracking-widest whitespace-nowrap">
                          ...
                          <TableCell className="text-right px-6 md:px-8 whitespace-nowrap">
                            <Button 
                              variant="outline" 
                              size="sm" 
                              className="bg-red-50 text-red-600 hover:bg-red-100 border-red-200 rounded-xl font-bold h-8 md:h-9 px-4 md:px-6 text-[10px]"
