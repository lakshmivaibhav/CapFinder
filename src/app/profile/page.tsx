              ...
              <div className="relative inline-block mx-auto mb-8 md:mb-10 mt-2 md:mt-4 cursor-pointer group/avatar" onClick={() => fileInputRef.current?.click()}>
                <div className="w-36 h-36 md:w-44 md:h-44 bg-muted/30 rounded-[2.5rem] flex items-center justify-center border-4 border-white shadow-xl md:shadow-2xl relative overflow-hidden transition-transform duration-500 group-hover/avatar:scale-105">
                  {formData.photoURL ? (
                    ...
                    <User className="text-muted-foreground opacity-20 w-16 h-16 md:w-20 md:h-20" />
                  )}
                  {uploading && (
                    <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-10 backdrop-blur-sm">
                      <Loader2 className="animate-spin text-white w-8 h-8 md:w-10 md:h-10" />
                    </div>
                  )}
              ...
              <div className="pt-8 md:pt-10 mt-8 md:mt-10 border-t border-dashed space-y-6 md:space-y-8">
                {formData.role === 'startup' && (
                  <div className="space-y-4 md:space-y-5">
                    ...
                    >
                      {formData.logoURL ? (
                        <Image src={formData.logoURL} alt="Logo" fill className="object-contain p-3 md:p-4 transition-transform group-hover/logo:scale-110" unoptimized />
                      ) : (
                        <div className="text-center p-3 md:p-4">
                          {uploadingLogo ? <Loader2 className="w-6 h-6 md:w-8 md:h-8 animate-spin text-primary" /> : <Upload className="w-6 h-6 md:w-8 md:h-8 text-muted-foreground opacity-10" />}
                        </div>
                      )}
